...
export function InstallationIosContent() {
  const iosGuide = installationGuideItems.find((item) => item.id === 'ios-pwa');

  if (!iosGuide) return null;

  return (
    <div className="animate-in fade-in slide-in-from-bottom-2 duration-500">
      <header className="mb-8">
        <h1 className="text-3xl font-bold tracking-tight text-slate-900 sm:text-4xl">
          Instalação no iOS (PWA)
        </h1>
        ...
        {iosGuide.note && (
          <div className="mt-8 rounded-xl border border-sky-100 bg-sky-50/50 p-4">
            <div className="flex gap-3">
              ...
              <p className="text-sm text-sky-900 leading-relaxed">{iosGuide.note}</p>
            </div>
          </div>
        )}
